...
function updateChannel(client) {
    ...
    if ($("#guilds").val() === 'DM') {
        ...
        $("#guildInfo").html(`User ID : (${user.id}) <button class='mini' value='<@!${user.id}>' onclick="addText(this.value)"> <i class="fas fa-at"></i> </button>`);///lord creative
///lord creative///lord creative
        $("#channelNameLabel").text(`Chat [${user.username}]`);///lord creative
        ...
        try {
            channel.fetchMessages()
                .then((messages) => {
                    ...
                    var html;
                    if ($("#guilds").val() === 'DM') {
                        msgArray.forEach((msg) => {///lord creative
...
function dlt(value) {///lord creative
    if (window.confirm('Token Değiştirmeye Emin misin ?')) {
