...
client.on("message", async msg => {
  const i = await db.fetch(`${msg.guild.id}.ghostkufur`);
  if (i) {
    const ghostkufur = [
      "oç",
      "amk",
    ...
    if (ghostkufur.some(word => msg.content.includes(word))) {
      try {
...
client.on("message", async message => {
  ...
  let reklamkick = await db.fetch(`ghostkufur_${message.guild.id}`);
